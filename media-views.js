...
// Check if the browser supports CSS 3.0 transitions
        ...
        media.view.yjzlib = __webpack_require__( 101 );//要顺着写
...
// Map some of the modal's methods to the frame.
    ...
    /***/ (function(module, exports) {
        ...
        Select = MediaFrame.extend(/** @lends wp.media.view.MediaFrame.Select.prototype */{
            ...
            bindHandlers: function() {
                ...
                this.on( 'content:render:yjzlib', this.yjzlibContent, this );
             ...
             * Render callback for the content region in the `upload` mode.
             */
            yjzlibContent: function() {
                this.$el.removeClass( 'hide-toolbar' );
                var state = this.state();

                this.content.set( new wp.media.view.yjzlib({
                    controller: this,
                    collection: state.get('library'),
                    selection:  state.get('selection'),
                    model:      state,
    ...
    /***/ (function(module, exports) {
        ...
        Post = Select.extend(/** @lends wp.media.view.MediaFrame.Post.prototype */{
            ...
            /**
             * Create the default states. //隐藏图库边框
             */
            createStates: function() {
                var options = this.options;
                    ...
                    items: {
                        insert: {
                            ...
                            click: function() {
                                var controller = this.controller;
                                var state = controller.state();
                                var edit = controller.state('gallery-edit');
    ...
    /***/ (function(module, exports) {
        ...
        var Button = wp.media.View.extend(/** @lends wp.media.view.Button.prototype */{
            ...
            click: function( event ) {
                ...
                var istrue = this.options.click && ! this.model.get('disabled');
                if ( !istrue) {
                    return
                }
                ...
                if(this.controller.content._mode == 'yjzlib')
                {
                    ...
                    if(result_data!=null)
                    {
                        for(var i=0;i<yjz_models.length;i++)
                        {
                            yjz_models[i].attributes.id = result_data[i].id;
            ...
            upload_yjz_img:function(yjz_models){

                var urls = new Array();

                //视频
                if(yjz_models.length==1 && yjz_models[0].attributes.type=='video')
                {
                    urls[0] = yjz_models[0].attributes.url;
                    var icon = yjz_models[0].attributes.icon;
                    var params = {'action':'yjz-import-attachment','urls':urls,'at_tpye':'video','thumbicon':icon };
                }else //图片
                {
                    for(var i =0 ;i< yjz_models.length;i++)
                    {
                        urls[i] = yjz_models[i].attributes.url;
                    }
                    var params = {'action':'yjz-import-attachment','urls':urls,'at_tpye':'image' };
    ...
    /***/ (function(module, exports) {
        ...
        Attachment = View.extend(/** @lends wp.media.view.Attachment.prototype */{
            ...
            toggleSelectionHandler: function( event ) {
                ...
                //如果选中就移除
                if(this.$el.hasClass( 'attachment' ) && this.$el.hasClass( 'selected' ))
                {
                    var selection = this.options.selection;
                    if ( selection ) {
...
// Ensure settings remain in sync between attachment views.
    ...
    /***/ (function(module, exports) {
        ...
        Attachments = View.extend(/** @lends wp.media.view.Attachments.prototype */{
            ...
            scroll: function() {
                ...
                //by jack 修复无图片数据加载图标显示异常
                var tempdata = el.scrollHeight - ( scrollTop + el.clientHeight );
                if ( tempdata!=0 && (tempdata < el.clientHeight / 3) ) {
    ...
    /***/ (function(module, exports) {
        ...
        AttachmentsBrowser = View.extend(/** @lends wp.media.view.AttachmentsBrowser.prototype */{
            ...
            updateContent: function() {
                ...
                //by jack
                if ( ! this.collection.length && view.collection.length >0) {
    ...
    /***/ (function(module, exports) {
         ...
         * @augments Backbone.View
         */
        yjzlib = View.extend(/** @lends wp.media.view.UploaderInline.prototype */{
            tagName:   'div',
            className: 'yjz-piclib-box',
            template:  wp.template('media-yjzlib'),

            events: {
                'click .close': 'hide',
                'click .yjz-pic-type':'pic_type_click',
                'click .yjz-system-pic':'pic_box_click'
            },
            initialize: function() {
                _.defaults( this.options, {
                    message: '',
                ...
                this.el.id = _.uniqueId('__yjz-piclib-box-');
                $(".yjz-piclib-box").remove();
            },
            ...
            ready: function() {
                if(this.readying!=true )
                {
                    ...
                    }else //image
                    {
                        $(".yjz-video-type").hide();
                        $(".yjz-pic-type").show();
                        load_pic_data('all','pic');
                        $(".yjz-piclib-toolbar").show();
                    }
                }
                this.readying = true;
                return this;
            ...
            loadNextPageData:function()
            {
                ...
                if (scrollTop!=0 && (scrollTop + divHeight  >= scrollHeight)) {
                    ...
                    if(atType =='image')
                    {
                        var picType = $("#yjz-pic-type").val();
                        load_pic_data(picType,'pic');
                    }else if(atType =='video')
                    {
                        var picType = $("#yjz-video-type").val();
                        load_pic_data(picType,'video');
                    }
                }
            },
            pic_type_click:function(event)
            {
                var yjz_pic_type=$(event.target).data('id');
                $(".yjz-pic-type").removeClass('yjz-active');
                $(event.target).addClass('yjz-active');
                load_pic_data(yjz_pic_type,'pic');
            },
            pic_box_click:function(event){
                var obj = $(event.target).parent().parent();
                var pic_url= $(obj).data('url');
                var thumb_icon = $(obj).data('thumbicon');

                var is_selected = $(obj).hasClass("details")||$(obj).hasClass("selected");
                event.stopPropagation();

                if( $("#yjz-state").val()=='true')
                {
                    this.pic_box_click_multiple(pic_url,is_selected,obj);

                }else
                {
                    this.pic_box_click_single(pic_url,is_selected,obj,thumb_icon);
                }
            },
            pic_box_click_single:function (pic_url,is_selected,obj,thumb_icon) {

                var collection = this.collection;
                //清空选项
                this.options.selection.models.splice(0, 1);
                if(is_selected)//被选中
                {
                    $(".yjz-system-pic-selected").removeClass('details selected yjz-system-pic-selected');
                    this.controller.toolbar.view.$el.find( '.media-button-select' ).prop( 'disabled', 'disabled' );
                    this.controller.state().attributes.selection.off( null, null, this );

                    // $(targetbtn).attr('disabled','disabled');
                    //  $(targetbtn).removeAttr('disabled');
                    // $(targetbtn).removeData('yjzpicurl');
                }else
                {
                    $(".yjz-system-pic-selected").removeClass('details selected yjz-system-pic-selected');
                    $(obj).addClass('details').addClass('selected').addClass('yjz-system-pic-selected');
                    this.controller.toolbar.view.$el.find( '.media-button-select' ).prop( 'disabled', '' );
                    this.options.selection.off( null, null, this );
                    var img_model =this.yjz_img_model(pic_url,thumb_icon);
                    this.options.selection['add'](img_model);
                    this.options.selection.single( img_model );
                    //  this.controller.state().attributes.selection.push(img_model);
                    // this.controller.state().attributes.selection['add'](img_model);

                    // $(targetbtn).data('yjzpicurl',url);
                    // $(targetbtn).removeAttr('disabled');
                }
